...
def main():
    ...
    if SAMPLE_IDX >= total_len:
        print(
            f"[Error] 인덱스 {SAMPLE_IDX}는 범위를 벗어났습니다. (최대 {total_len - 1})"
        )
        return
    ...
    with open(txt_path, "w", encoding="utf-8") as f:
        ...
        f.write(">>> Next Target Audio (Right Channel) plays now.")

    print("\n>>> [완료]")
    print(f"1. 오디오 파일: {wav_path}")
    print("   ★ 듣는 법:")
    print(
        "   - 왼쪽 소리(User)가 쭉 나오다가, 끝나면 오른쪽 소리(Target)가 '삑' 하고 나옵니다."
    )
    print("   - 이것이 모델이 학습하는 [Input -> Output] 관계입니다.")
