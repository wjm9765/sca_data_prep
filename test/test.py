...
def print_sample_details(idx, sample):
    ...
    print("   (Expectation: Step이 지날수록 길이가 2씩(User+Text) 늘어나야 함)")
    ...
    if target_wav.shape[0] == expected_samples:
        print(
            f"   >>> [PASS] Target Shape matches 24kHz 4-token spec ({expected_samples})."
        )
    else:
        print(
            f"   >>> [WARNING] Target Shape mismatch! Expected {expected_samples}, got {target_wav.shape[0]}"
        )
