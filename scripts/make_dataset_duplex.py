...
try:
    ...
    print(
        "[Error] we can't find the 'sca_data' package. Please ensure the 'src' directory is added to PYTHONPATH."
    )
    sys.exit(1)
...
def save_parsed_scripts(data_dir: Path, output_dir: Path):
    ...
    if not txt_dir.exists():
        raise FileNotFoundError(
            f"TXT folder not found in {data_dir}. Please check your input path."
        )
...
def build_hf_dataset(data_dir: Path, output_dir: Path):
    ...
    if not (data_dir / "WAV").exists() or not (data_dir / "TXT").exists():
        raise FileNotFoundError(
            f"Input directory must contain 'WAV' and 'TXT' folders.\nChecked: {data_dir}"
        )
    duplex_data(data_dir=data_dir, cache_dir=output_dir)
...
def main():
    ...
    if not input_path.exists():
        ...
        print(
            "Please ensure the 'Multi-stream Spontaneous Conversation Training Dataset' folder exists in the root."
        )
        sys.exit(1)
