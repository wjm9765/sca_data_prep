...
def download_videos_from_search(
    search_result: SearchResult, output_path: str = ".", only_audio=False
) -> None:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Download YouTube videos from search results."
    )
    parser.add_argument(
        "search_results", type=str, help="Search results JSON file path."
    )
    parser.add_argument(
        "--download-count",
        type=int,
        default=None,
        help="Number of videos to download from the search results.",
    )
    parser.add_argument(
        "--shuffle",
        action="store_true",
        help="Shuffle the order of videos before downloading.",
    )
    parser.add_argument(
        "--output-path",
        type=str,
    ...
    parser.add_argument(
        "--only-audio", action="store_true", help="Download only audio from the videos."
    )
    ...
    search_result = SearchResult.model_validate_json(
    ...
    if args.download_count is not None:
        if int(args.download_count) > len(search_result.results):
            warnings.warn(
                f"Requested download count {args.download_count} exceeds available results {len(search_result.results)}. Downloading all available results."
            )
        search_result.results = search_result.results[: args.download_count]
    download_videos_from_search(
        search_result, output_path=str(output_path), only_audio=args.only_audio
    )
